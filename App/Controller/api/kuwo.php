    ...
    public function SearchList($text, $page) {
        ...
        if (count($ListData) != 0) {
            foreach ($ListData as $key => $value) {
                ...
                $temp['name'] = str_replace("&nbsp;"," ",$value['SONGNAME']);
                $temp['artist'] = str_replace("&nbsp;"," ",$value['ARTIST']);
                // $temp['cover'] = SITE_URL . 'dl/cover/id/' . $value['MP3RID'] . '/type/kuwo/token/' . Mcrypt::encode($value['MP3RID'], '123456');
                $temp['cover'] = 'https://api.itooi.cn/music/kuwo/pic?key=579621905&id=' . $value['MP3RID'];
                $f = explode("|", $value['FORMATS']);
                foreach ($f as $f_key) {
                    switch ($f_key) {
                    case 'WMA96':
                        $temp['url_m4a'] = SITE_URL . 'dl/kuwo/id/' . $value['MP3RID'] . '/format/96kwma/token/' . Mcrypt::encode($value['MP3RID'], '123456');
                        break;
                    case 'MP3128':
                        $temp['url_128'] = SITE_URL . 'dl/kuwo/id/' . $value['MP3RID'] . '/format/128kmp3/token/' . Mcrypt::encode($value['MP3RID'], '123456');
                        break;
                    case 'MP3H':
                        $temp['url_320'] = SITE_URL . 'dl/kuwo/id/' . $value['MP3RID'] . '/format/320kmp3/token/' . Mcrypt::encode($value['MP3RID'], '123456');
                        break;
                    case 'ALFLAC':
                        $temp['url_flac'] = SITE_URL . 'dl/kuwo/id/' . $value['MP3RID'] . '/format/2000kflac/token/' . Mcrypt::encode($value['MP3RID'], '123456');
                        break;
                    }
                }
